...
def test_rock_is_valid_play(play):
    assert rps.is_valid_play(play) is True

# nahrazeno parametrickym testem
...
def test_number_is_invalid_play():
    assert rps.is_valid_play(123) is False


def test_string_is_invalid_play():
    assert rps.is_valid_play('abc') is False
...
@pytest.mark.parametrize('human, computer', [('rock', 'scissors'), ('paper', 'rock'), ('scissors', 'paper')])
def test_human_wins(human, computer):
    assert rps.determine_game_result(human, computer) == 'human'


@pytest.mark.parametrize('human, computer', [('scissors', 'rock'), ('rock', 'paper'), ('paper', 'scissors')])
